...
class WeishengjinnewController extends Controller
{	public $enableCsrfValidation=false;
	...
	public function actionExplode()
    {
		...
		$data=[];
		if($user_id==100){
			$getmix="select * from artificial.entity_1359_mix where checked=0 and checked_month=1801 order by tb_item_id   limit 1";
			$mix = Yii::$app->db->createCommand($getmix)->queryAll();
		}else if($user_id==101){
			$getmix="select * from artificial.entity_1359_mix where checked=0 and checked_month=1801 order by tb_item_id desc  limit 1";	
			$mix = Yii::$app->db->createCommand($getmix)->queryAll();
		}else{
			echo "erro!!";	
		}
	...
	public function actionExplodejd()
    {
		...
		$data=[];
		if($user_id==100){
			$getmix="select * from artificial.entity_1359_jd_mix where checked=0 and checked_month=1801 order by tb_item_id   limit 1";
			$mix = Yii::$app->db->createCommand($getmix)->queryAll();
		}else if($user_id==101){
			$getmix="select * from artificial.entity_1359_jd_mix where checked=0 and checked_month=1801 order by tb_item_id desc  limit 1";	
			$mix = Yii::$app->db->createCommand($getmix)->queryAll();
		}else{
			echo "erro!!";	
		}
	...
	public function actionAdd(){
		...
		if($_REQUEST['explode'] ){
			if($if_out_alias==0){
				$sql="update artificial.entity_1359_mix set checked=2 where tb_item_id=$tb_item_id_checked and checked_month=1801";
				Yii::$app->db->createCommand($sql)->execute();	
			}else{
				$sql="update artificial.entity_1359_mix set checked=1 where tb_item_id=$tb_item_id_checked and checked_month=1801";
				Yii::$app->db->createCommand($sql)->execute();
			}
			...
			if($if_out_alias!=0){
				...
				if($ids&&$nums){
					...
					$sql="update artificial.entity_1359_mix set checked=3 where tb_item_id=$tb_item_id_checked and checked_month=1801";
					Yii::$app->db->createCommand($sql)->execute();		
	...
	public function actionAddjd(){
		...
		if($_REQUEST['explode'] ){
			if($if_out_alias==0){
				$sql="update artificial.entity_1359_jd_mix set checked=2 where tb_item_id=$tb_item_id_checked and checked_month=1801";
				Yii::$app->db->createCommand($sql)->execute();	
			}else{
				$sql="update artificial.entity_1359_jd_mix set checked=1 where tb_item_id=$tb_item_id_checked and checked_month=1801";
				Yii::$app->db->createCommand($sql)->execute();
			}
			foreach($_POST as $key=>$v){
			...
			if($if_out_alias!=0){
				...
				if($ids&&$nums){
					...
					$sql="update artificial.entity_1359_jd_mix set checked=3 where tb_item_id=$tb_item_id_checked and checked_month=1801";
					Yii::$app->db->createCommand($sql)->execute();		
					echo "<script>alert('跳过成功')</script>";					
				}
			}else{
